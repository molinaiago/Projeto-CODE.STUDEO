...
import avanc from "../../data/trilha-avancada.json";
...
/* CodeJS (mesmo da intermediária) */
function CodeJSExercise({ atividade, onSolved }) {
  const [code,setCode]=useState(atividade.assinatura || "");
  const [result,setResult]=useState(null);

  const runTests=()=>{
    const logs=[]; const fails=[];
    try{
      const wrapped = `(function(){ ${code}; return {${atividade.exportName || "solve"}}; })()`;
      const mod = Function('"use strict"', `return ${wrapped}`)();
      const fnName = atividade.exportName || "solve";
      if(typeof mod[fnName] !== "function"){
        setResult({ok:false, logs, fails:[`Função ${fnName} não encontrada.`]}); return;
      }
      ...
      setResult({ok:false, logs:[], fails:[`Erro: ${err.message}`]});
    }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <textarea
        className="min-h-[180px] rounded-xl border border-white/10 bg-zinc-950 px-3 py-3 text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-500/60"
        value={code}
        onChange={e=>setCode(e.target.value)}
      />
      <button onClick={runTests} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-semibold">
        Executar testes
      </button>
      ...
      {result && (
        <div className={`rounded-xl border px-4 py-3 text-sm ${result.ok?"border-emerald-500/30 bg-emerald-500/10 text-emerald-200":"border-amber-500/30 bg-amber-500/10 text-amber-200"}`}>
          <div className="font-semibold mb-1">{result.ok?"Todos os testes passaram!":"Falhas nos testes"}</div>
...
/* Página */
export default function ExercicioAvancada(){
  const { id } = useParams();
  const { isActivityUnlocked, markExerciseDone } = useProgress();

  const data = findExerciseById(avanc, id);
  if (!data) {
    return (
      <main className="min-h-[calc(100vh-4rem)] bg-zinc-950 text-zinc-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/trilha/avancada" className="mt-3 inline-block underline text-zinc-300">Voltar</Link>
        </div>
      </main>
    );
  }
  ...
  const moduloIndex = avanc.modulos.findIndex(m => m.moduloId===modulo.moduloId);
  const atividadeIndex = avanc.modulos[moduloIndex].atividades.findIndex(a=>a.id===atividade.id);
  const canOpen = isActivityUnlocked("avancada", avanc, moduloIndex, atividadeIndex);
    ...
    return (
      <main className="min-h-[calc(100vh-4rem)] bg-zinc-950 text-zinc-100 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <Link to="/trilha/avancada" className="mt-4 inline-block underline text-zinc-300">← Voltar para a trilha</Link>
        </div>
      </main>
    );
  }

  const handleSolved = () => markExerciseDone("avancada", atividade.id);

  return (
    <main className="min-h-[calc(100vh-4rem)] bg-zinc-950 text-zinc-100">
      <section className="max-w-3xl mx-auto px-4 md:px-6 py-10">
        <Link to="/trilha/avancada" className="text-sm text-zinc-300 underline">← Voltar para {modulo.titulo}</Link>
        ...
        <div className="mt-6 rounded-2xl border border-white/10 bg-zinc-900/40 p-5">
          ...
          {/* acrescente outros tipos se quiser reaproveitar */}
        </div>
      </section>
    </main>
  );
